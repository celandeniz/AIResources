...
class Meeting {
  ...
  factory Meeting.fromJson(Map<String, dynamic> j) {
    ...
    return Meeting(
      ...
      start: m['start'] != null
          ? DateTime.tryParse(m['start'] as String)?.toLocal()
          : null,
      end: m['end'] != null
          ? DateTime.tryParse(m['end'] as String)?.toLocal()
          : null,
      attendees: (m['attendees'] as List? ?? const [])
          .map((e) => e.toString())
          .toList(),
  ...
  Future<void> _act(
    ...
    try {
      await ref
          ...
          .api
          .post('/meetings/$id/$action', body: body);
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      PageHeader(
                        title: 'Toplantılar',
                        subtitle: 'Takvim kararları',
                      ),
                      EmptyState(
                        icon: Icons.event_available_outlined,
                        title: 'Bekleyen toplantı onayı yok',
                      ),
                    ],
                  ),
                ),
                              ...
                              Wrap(
                                ...
                                children: [
                                  DynButton(
                                    ...
                                    onPressed: _busyIds.contains(m.id)
                                        ? null
                                        : () => _act(
                                            m.id,
                                            'reject',
                                            body: {
                                              'note': 'Mobilden reddedildi',
                                            },
                                          ),
                                    child: const Text('Reddet'),
                                  ),
                                  DynButton(
                                    variant: DynButtonVariant.outline,
                                    size: DynButtonSize.sm,
                                    onPressed: _busyIds.contains(m.id)
                                        ? null
                                        : () => _proposeTime(m),
                                    child: const Text('Alternatif zaman'),
                                  ),
                                  DynButton(
